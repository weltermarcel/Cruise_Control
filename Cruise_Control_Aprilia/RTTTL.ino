...
int notes[] = {0, 262, 277, 294, 311, 330, 349, 370, 392, 415, 440, 466, 494, 523, 554, 587, 622,
...
void play_rtttl(char *p) {
	...
	while (*p)
	{
		...
		if (note)
		{
			...
			if (Flag_Servo_Music == true) {
				Serial.println((map(notes[(scale - 4) * 12 + note], min_tone_frequency, max_tone_frequency, min_pos, max_pos) + tone_offset));
				throttle.write(map(notes[(scale - 4) * 12 + note], min_tone_frequency, max_tone_frequency, min_pos, max_pos) + tone_offset);
			}
